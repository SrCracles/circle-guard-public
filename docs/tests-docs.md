...
Las pruebas unitarias validan la lógica interna de cada componente aislando sus dependencias mediante _mocks_.
...
- **`AdminControllerTest`**: Pruebas de configuración del sistema (SystemSettings) y toggle de _unconfirmed fencing_.
...
**Ejecución actual en Jenkins:** el stage elimina primero cualquier pod huérfano `zap-scan`, vuelve a crearlo y ejecuta el script con `sh /tmp/run-zap-scans.sh`. Esto evita fallos por `AlreadyExists` cuando una corrida anterior dejó el pod vivo.

**Compatibilidad de archivos:** `tests/zap/run-zap-scans.sh` es POSIX `sh` y el repositorio fuerza finales de línea LF para que el script se ejecute correctamente al copiarse desde Jenkins en Windows hacia el contenedor Linux.
